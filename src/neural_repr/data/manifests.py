...
of a dataset split: one row per image with its split ID, relative path, SHA-256, and
inspected properties (format, shape, color mode). The image *bytes* are never
...
corrupt/unreadable files, duplicate hashes, and **train/test split leakage** (the
same image content appearing in more than one split). These run against a local
dataset directory and, for the committed synthetic fixture, in CI.
"""
...
def build_manifest_row(root: Path, path: Path, *, split_id: str, role: str) -> ManifestRow:
...
def validate_manifest(
    ...
    *,
    ...
    for r in rows:
        ...
        if not _SHA256_RE.match(r.sha256):
    ...
    # Split leakage: identical content hash in more than one role.
    by_hash: dict[str, set[str]] = {}
    for r in rows:
        by_hash.setdefault(r.sha256, set()).add(r.role)
...
def verify_manifest_on_disk(
    ...
    *,
    ...
    height, width, mode, and SHA-256 to the recorded row. Optionally enforces an exact
    set of expected roles / split IDs (from a registry spec). Returns a problem list.
    """
    ...
    for r in rows:
        ...
        if actual != r.sha256:
            problems.append(
                f"{r.split_id}: content hash mismatch — disk {actual[:19]}... != manifest "
                f"{r.sha256[:19]}..."
            )
    return problems
