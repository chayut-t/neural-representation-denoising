...
**Plan status:** Phases 0–3 complete (Gate P2 passed and independently verified 2026-07-18; Gate P3 passed 2026-07-19 — formalized, finite-difference-tested mathematics for both studies in `src/neural_repr/{sparse_coding,attractor}`). Phase 4 code (data registry, on-disk validation, color/whitening/corruption/patch pipeline, config-driven train-only fit/apply, synthetic fixture) is complete and green, but **Gate P4 is OPEN**: its real DIV2K/BSDS500 manifests and provenance-bound audit require authorized download and are deferred to Phase 5 entry by decision 0010, which also blocks Phase 5 on Gate P4. The locked environment (`uv.lock`), public reference containers, the LuaLaTeX `dissertation/` working edition, and CI with layered leak prevention are green on a clean checkout; the 2026 rewrite remains frozen (tag `archive/rewrite-2026-baseline`). No reproduced scientific *results* exist yet (no confirmatory run has been executed)
...
| Phase 3 | Complete (Gate P3 passed 2026-07-19) | Formalized, finite-difference-tested mathematics for both studies in `src/neural_repr/{sparse_coding,attractor}`; math spec, docstrings, notation, and tests agree |
| Phase 4 | Code complete; **Gate P4 OPEN** | Data registry, registry-aware on-disk validation, color/whitening/corruption/patch pipeline, config-driven train-only fit/apply, and synthetic fixture are done and green; the real DIV2K/BSDS500 manifests + provenance-bound audit require authorized download and are deferred to Phase 5 entry (decision 0010) |
| Phase 5 | Blocked on Gate P4 (decision 0010) | Reimplement + validate the sparse-coding study on controlled data |
| Phases 6-11 | Pending | No reproduced runs or generated empirical artifacts exist yet |
...
**Gate P2 - PASSED (2026-07-18; re-verified 2026-07-19 after codex review):** Verified from a fresh clone — `uv sync --locked` (lockfile unchanged), package import, full test suite passing, all six CLI `--help` commands, and the LuaLaTeX `dissertation/` edition building via the canonical non-overwriting command to `builds/dissertation/<build-id>/` (dissertation/ left clean) with no undefined references/citations (57 vs 58 pages, a documented benign font/bibliography reflow); both `legacy/` baseline manifests validate; the lineage checker enforces one FILE_MAP row per derived file against `docs/baseline-audit-2026.md` (16 figure + 6 table labels). The public CPU Dockerfile builds without private credentials and its `neural-repr-verify check-image` passes (import, closed provenance-schema, numerical-smoke, synthetic-canary leak-scan). The CUDA Dockerfile is digest-pinned to a CUDA-13.0 base matching the locked torch 2.13.0/CUDA-13.0 wheels; its build-time stack assertion runs on the amd64 CI runner.

**CUDA build-time vs runtime split (decision 0009):** the P2 CUDA criterion is the **build-time** check — the public CUDA image builds from digest-pinned public bases and `check-image` (no `--require-cuda`) passes import, closed provenance-schema, CPU numerical-smoke, synthetic-canary leak-scan, and the exact python/torch/CUDA **version assertions** (`--expect-python 3.11.15 --expect-torch 2.13 --expect-cuda 13.0`) on the amd64 CI runner. The CUDA **runtime** self-check (`check-image --require-cuda`, a real GPU op) is reassigned by decision 0009 to a **blocking Gate P13 criterion** (the `cuda-runtime-check` CI job, gated on `vars.GPU_RUNNER_LABEL`, is its mechanized form) — it is not a dangling P2 pending item, and no GPU experiment runs before Phase 13. Layered leak prevention (closed nested-schema allow-list + generic scanner over all text incl. Dockerfiles + gitleaks) and a scheduled base+uv pullability check are wired; no infrastructure-identifying value appears in any tracked file. Deferred to later phases (not P2 blockers): `LICENSE`/`CITATION.cff`/`Makefile` and the exact code-license string.
...
- the public CUDA image passes the **build-time** checks: import, CPU numerical-smoke, provenance-schema, synthetic-canary leak-scan, and the exact python/torch/CUDA version assertions (the **runtime** GPU self-check is a Gate P13 criterion per decision 0009);
...
**Gate P4 - OPEN (reopened 2026-07-19 after round-2 review; the earlier "passed" was withdrawn).** The CPU preprocessing machinery, registry, on-disk validation, config-driven train-only fit/apply pipeline, immutable archive/terms records, and synthetic-fixture reproducibility are implemented and green. Two criteria remain genuinely unmet and **require authorized dataset access** to satisfy: (a) the committed *scientific* split manifests `data/manifests/div2k.csv` / `data/manifests/bsds500.csv` (task 5) do not exist because DIV2K/BSDS500 bytes cannot be downloaded/committed here; and (b) a provenance-bound audit over those real manifests (task 11, incl. before/after sample images). **Decision 0010** formally defers exactly those two dataset-materialization artifacts to the start of Phase 5 (authorized-access boundary) and records that Phase 5 is blocked on them. Everything that does not need the real bytes is done and verified on the synthetic fixture:

- registry (DIV2K/BSDS500 official URLs, dataset-specific terms tokens, fixed splits — decision 0001); immutable terms-acceptance (URL + token + terms content hash) and **create-only** archive-hash pins (a different digest fails; rotation is a separate audited step); dataset-specific acceptance flags;
- registry-aware **on-disk** `neural-repr-data check`: resolves each row's path under a root with traversal protection, enforces registry roles/IDs/counts, validates digest syntax, and reopens/decodes/rehashes every image, comparing actual format/shape/mode/SHA-256 to the row before duplicate/leakage checks;
- committed synthetic manifest carrying, per image, a platform-stable decoded-pixel digest (`content_sha256`) plus an optional file-byte `sha256` — the fixture pins only the pixel digest (regenerated fresh per platform; PNG/zlib encoding is not a cross-platform byte invariant), while downloaded third-party data keeps full file-byte hashes; validation, split-leakage, and duplicate checks all key on the reproducible pixel digest;
- color/whitening/corruption/patch pipeline: sRGB↔linear, orthonormal opponent-color, DC removal, ZCA whitening (fit/transform/inverse), global vs per-patch normalization, AWGN by target SNR and fixed sigma, deterministic `(dataset hash, image id, seed, patch index)` sampling, flips+90° augmentation only, overlapping extract/reassemble with normalized Hann blending — every inverse within `tol-preproc-inverse`;
- a **config-driven train-only fit/apply** entry point that selects training rows internally, rejects a non-training row at the fit boundary, digests the exact fit subset+config, and writes a non-overwriting provenance-bound stats artifact; frozen stats apply to val/test;
- no-overwrite writers (collision-failing atomic creates) across the data writers and the dissertation build;
- Hydra dataset/preprocessing configs and `data/README.md` (license handling).

**Gate P4 will be marked passed** only once decision 0010's deferred artifacts exist (real manifests + provenance-bound audit with acceptance thresholds) and an independent clean-checkout verification passes.

The original criteria were:

Re-running preprocessing yields identical manifests and arrays on the reference platform; inverse transforms reconstruct inputs within tolerance; no test image influences fitted preprocessing; license handling is documented.
...
**Gate P13:** A clean independent run on the public reference environment produces a valid dissertation and all expected artifacts; deviations are within documented tolerances; **every reported value traces to a public-reference run or a passed cross-environment equivalence record**, and the release manifest fails the gate if any value can be traced to neither; the **CUDA runtime self-check** (`check-image --require-cuda` with the exact python/torch/CUDA version assertions — the `cuda-runtime-check` CI job) passes on the public reference GPU environment (reassigned here from Phase 2 by decision 0009); the verifier signs the release checklist.
...
Phases 0–3 have passed (Gates P1/P2 independently verified 2026-07-18; Gate P3 passed 2026-07-19). Phase 4 code is complete but **Gate P4 is open** pending its authorized-download dataset artifacts (decision 0010). The completed artifacts — the frozen `legacy/rewrite-2026/` tree and tag `archive/rewrite-2026-baseline`; `docs/rewrite-2026-inventory.csv`, `docs/baseline-audit-2026.md`, `docs/claim-evidence-matrix.md`, `docs/decisions/`, `docs/tolerances.md`; the `src/neural_repr/` package (incl. the formalized `sparse_coding`/`attractor` math and the `data` pipeline), committed `uv.lock`, `containers/`, the `dissertation/` working edition, and CI — are inputs to the remaining work. Do not mutate the frozen baselines.

The next action is to **close Gate P4**: after an authorized DIV2K/BSDS500 download (the `accept-terms` → download → `verify-archive` flow), build and commit the real split manifests and run the provenance-bound audit (decision 0010), then obtain independent clean-checkout verification before marking P4 passed. Only then begin **Phase 5**. Do not begin expensive experiments until Gate P4 passes. Thereafter execute phases in order and stop at every gate whose acceptance criteria are not met.
