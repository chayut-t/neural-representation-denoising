...
**PENDING (does not block P2, tracked):** the CUDA *runtime* self-check (`check-image --require-cuda`, a real GPU op) needs a GPU-capable runner; the `cuda-runtime-check` CI job is gated on one and does not run until it exists. Layered leak prevention (closed-schema allow-list + generic scanner over all text incl. Dockerfiles + gitleaks) and a scheduled base+uv pullability check are wired; no infrastructure-identifying value appears in any tracked file. Deferred to later phases (not P2 blockers): `LICENSE`/`CITATION.cff`/`Makefile` and the exact code-license string.
