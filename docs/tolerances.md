...
| tol-preproc-repro | reproducibility | re-running preprocessing / regenerating the synthetic fixture yields identical pixel arrays and identical manifest `content_sha256` digests (decoded-pixel hash) **on any platform** | exact (bit-identical pixels + content digest, cross-platform). NB: PNG-encoded *file bytes* are **not** guaranteed cross-platform (zlib varies by build); the fixture pins no file-byte hash, so `sha256` is empty for it. Downloaded third-party file/archive byte hashes remain exact byte-integrity checks. | committed | maintainer | Phase 4 |
